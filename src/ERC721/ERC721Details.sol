...
abstract contract ERC721Details is IERC721Details, ERC721 {
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return interfaceId == type(IERC721Details).interfaceId || super.supportsInterface(interfaceId);
    }
    ...
    function _updateDetails(ContractDetails memory newContractDetails) internal virtual {
    ...
    function _generateURI(ContractDetails memory details_, uint256 sellFee, address feeRecipient)
        private
        pure
        returns (string memory uri)
    {
        ...
        return string(abi.encodePacked("data:application/json;base64,", Base64.encode(buffer)));
    }
}
